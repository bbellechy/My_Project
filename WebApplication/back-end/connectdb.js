...
router.post('/product_info', function (req,res){
    ...
    if (!product_info){
        return res.status(400).send({error: true,
             messege: " Please provide product infomation"});
    }
    connection.query("insert into product_sistem set ? ",
     product_info, function (error, results){
            if (error)
              return res.send({
                error: product_info,
...
router.put('/product_info', function (req,res){
    ...
    if (!product_info || !productID){
        return res.status(400).send(
            {error: true, messege: " Please provide product infomation"});
    }
    connection.query("update product_sistem set ? where id_product = ?", 
    [product_info,productID], function (error, results){
        if(error) throw error;
        return res.send({
            error: false,
            data: results.affectedRows,
...
router.delete('/product_info', function (req,res){
    ...
    if (!productID ){
        return res.status(400).send({error: true,
             messege: " Please provide ID product"});
            }
    connection.query("DELETE from product_sistem where id_product = ?",
